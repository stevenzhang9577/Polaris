...
async def read_batch(session, batch, user=None) -> SummaryBatchRead:
    ...
    return SummaryBatchRead(
        ...
        **{
            ...
            for key in ("pending", "running", "completed", "skipped", "failed", "cancelled")
        },
...
async def control_batch(session, *, batch, action):
    # Serialize controls with dispatch/completion; terminal cancellation cannot be resumed.
    await session.refresh(batch, with_for_update=True)
    if batch.status == "cancelled":
        return
    if action == "cancel" and batch.status in {"queued", "running", "paused"}:
        ...
        # Mark only revisions generated by this batch's owner; preserve ready summaries
        # and another user's generation that the batch was merely observing.
        await session.execute(
            update(PaperWikiRevision)
            .where(PaperWikiRevision.id.in_(
                select(SummaryBatchItem.revision_id).where(
                    SummaryBatchItem.batch_id == batch.id,
                    SummaryBatchItem.status == "cancelled",
                )), PaperWikiRevision.created_by == batch.user_id,
                PaperWikiRevision.status.in_(("queued", "generating")))
            .values(status="failed", stage=None, error_code="SUMMARY_CANCELLED",
                    error_detail="SUMMARY_CANCELLED")
        )
        await session.flush()
        return
...
async def finish_item(session, item_id, status):
    await session.execute(
        update(SummaryBatchItem)
        .where(SummaryBatchItem.id == item_id, SummaryBatchItem.status == "running")
    ...
    async with hold_capacity(lease_id), get_sessionmaker()() as session:
        ...
        try:
            ...
            if awaited is not None and awaited.status in {"ready", "stale"}:
                await finish_item(session, item_id, "completed")
                await session.commit()
                return
            ...
            if batch.skip_existing and current is not None and current.content:
                ...
                if revision is None or not await paper_summaries.revision_is_stale(
                    ...
                    await finish_item(session, item_id, "skipped")
                    await session.commit()
                    return
            ...
            if revision.created_by != user.id:
                ...
                await finish_item(session, item_id, "pending")
                await session.commit()
                return
            await session.commit()
            ...
            await session.rollback()
            cancelled_batch = await session.get(SummaryBatch, batch_id, populate_existing=True)
            if cancelled_batch is not None and cancelled_batch.status == "cancelled":
                # The revision may have been queued just after the cancel transaction.
                # Close that race without changing another owner's shared generation.
                await session.execute(
                    update(PaperWikiRevision)
                    .where(
                        PaperWikiRevision.id.in_(select(SummaryBatchItem.revision_id).where(
                            SummaryBatchItem.id == item_id,
                        )),
                        PaperWikiRevision.created_by == cancelled_batch.user_id,
                        PaperWikiRevision.status.in_(("queued", "generating")),
                    )
                    .values(status="failed", stage=None, error_code="SUMMARY_CANCELLED",
                            error_detail="SUMMARY_CANCELLED")
